 ...
 1111 format(2(2x,e11.5),2x,i2,2(2x,e11.5))      
      do i=1,nlat
        ...
        do j=1,nlat
          ...
          argepsm(j,i) = epsmax + (epsmin-epsmax)*(j-1.0d0)/(nlat-1.0d0)
          argmum(j,i) = rmumin + (rmumax-rmumin)*(i-1.0d0)/(nlat-1.0d0)

          argkm = argmum(j,i)
          argkp = (argepsm(j,i)+argkm)/2
          call test_isin(argkp,argkm,isin(j,i))

          zeps(1) = exp(ima*argepsm(j,i))
          zeps(2) = exp(ima*argkm)
          zmu(1) = exp(ima*argkm)
          zmu(2) = exp(ima*argkm)
          if(isin(j,i).eq.1) then
            call comp_thet(argkp,argkm,thet)

            z1 = exp(-ima*thet)*zeps(1)
            if(imag(z1).lt.0) then
              call prinf('poor choice of thet1=*',i,0)
              call prin2('zeps(1)=*',zeps(1),2)
              call prin2('z=*',z1,2)
              call prin2('argkp=*',argkp,1)
              call prin2('argkm=*',argkm,1)
            endif
            
            z1 = exp(-ima*thet)*zeps(2)
            if(imag(z1).lt.0) then
            ...
            z1 = exp(-ima*thet)*zeps(2)
            rphi = atan2(imag(z1),real(z1))
            zepssq(2) = exp(ima*thet/2)*exp(ima*rphi/2)
     ...
     1      nnmax,zlamall,singall,errall)
          ...
          enddo
          rcond(j,i) = rmax/rmin
          write(81,1111) argmum(j,i),argepsm(j,i),isin(j,i),
     1       rcond(j,i),err1
        enddo
      enddo
...
c
      ...
      alpha = rkm - rkp
      if(alpha.gt.0.and.alpha.le.pi/2.and.rkp.gt.0.and.
     1    rkp.lt.pi/2) thet = rkp
      if(alpha.gt.0.and.alpha.le.pi/2.and.rkp.ge.pi/2.and.
     1     rkp.lt.pi) thet = rkp-pi + 2*alpha
      if(alpha.ge.-pi/2.and.alpha.lt.0.and.rkp.gt.pi/2.and.
     1    rkp.lt.pi) thet = rkp-pi
      if(alpha.ge.-pi/2.and.alpha.lt.0.and.rkp.gt.-alpha
     1        .and.rkp.le.pi/2) thet = rkp+ 2*alpha
